...
// @ts-ignore
// https://github.com/bia-pain-bache/Ralysnd-Worker-Panel
...
// https://www.nslookup.io/domains/Ralysnd.yousef.isegaro.com/dns-records/
const proxyIPs= ['Ralysnd.yousef.isegaro.com'];
const defaultHttpPorts = ['80', '8080', '2052', '2082', '2086', '2095', '8880'];
const defaultHttpsPorts = ['443', '8443', '2053', '2083', '2087', '2096'];
let proxyIP = proxyIPs[Math.floor(Math.random() * proxyIPs.length)];
let dohURL = 'https://cloudflare-dns.com/dns-query';
let trojanPassword = `Ralysnd-trojan`;
            ...
            if (!upgradeHeader || upgradeHeader !== 'websocket') {
                ...
                switch (url.pathname) {
                    ...
                    case '/panel':
                        ...
                        if (typeof env.Ralysnd !== 'object') {
                            const errorPage = renderErrorPage('KV Dataset is not properly set!', null, true);
                            return new Response(errorPage, { status: 200, headers: {'Content-Type': 'text/html'}});
                        }

                        const pwd = await env.Ralysnd.get('pwd');
                        ...
                        const proxySettings = await env.Ralysnd.get('proxySettings', {type: 'json'});
                    ...
                    case '/login':
                        ...
                        let secretKey = await env.Ralysnd.get('secretKey');
                        if (!secretKey) {
                            secretKey = generateSecretKey();
                            await env.Ralysnd.put('secretKey', secretKey);
                        }

                        if (request.method === 'POST') {
                            const password = await request.text();
                            const savedPass = await env.Ralysnd.get('pwd');
                    ...
                    case '/panel/password':
                        ...
                        const oldPwd = await env.Ralysnd.get('pwd');
                        ...
                        await env.Ralysnd.put('pwd', newPwd);
...
async function updateDataset (env, Settings, resetSettings) {
    ...
    if (!resetSettings) {
        try {
            currentProxySettings = await env.Ralysnd.get("proxySettings", {type: 'json'});
        } catch (error) {
            console.log(error);
            throw new Error(`An error occurred while getting current values - ${error}`);
        }
    } else {
        await env.Ralysnd.delete('warpConfigs');
    }
    ...
    try {    
        await env.Ralysnd.put("proxySettings", JSON.stringify(proxySettings));          
    } catch (error) {
        console.log(error);
...
async function Authenticate (request, env) {
    ...
    try {
        const secretKey = await env.Ralysnd.get('secretKey');
...
async function renderHomePage (env, hostName, fragConfigs) {
    ...
    try {
        proxySettings = await env.Ralysnd.get('proxySettings', {type: 'json'});
        warpConfigs = await env.Ralysnd.get('warpConfigs', {type: 'json'});
        password = await env.Ralysnd.get('pwd');
    } catch (error) {
        console.log(error);
	...
	<head>
        ...
        <title>Ralysnd Panel ${panelVersion}</title>
	...
	<body>
		<h1>Ralysnd Panel <span style="font-size: smaller;">${panelVersion}</span> </h1>
		<div class="form-container">
			...
			<div class="table-container">
				<table id="normal-configs-table">
					...
					<tr>
						...
						<td>
                            <button onclick="openQR('https://${hostName}/sub/${userID}#Ralysnd-Normal', 'Normal Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/sub/${userID}#Ralysnd-Normal', false)">
					...
					<tr>
						...
						<td>
                            <button onclick="copyToClipboard('https://${hostName}/sub/${userID}?app=singbox#Ralysnd-Normal', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
						</td>
					</tr>
                    <tr>
                        ...
                        <td>
                            <button onclick="openQR('sing-box://import-remote-profile?url=https://${hostName}/sub/${userID}?app=sfa#Ralysnd-Normal', 'Normal Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/sub/${userID}?app=sfa#Ralysnd-Normal', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <button onclick="openQR('https://${hostName}/sub/${userID}?app=clash#Ralysnd-Normal', 'Normal Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/sub/${userID}?app=clash#Ralysnd-Normal', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
                    </tr>
			...
			<div class="table-container">
                <table id="frag-sub-table">
                    ...
                    <tr>
                        ...
                        <td>
                            <button onclick="openQR('https://${hostName}/fragsub/${userID}#Ralysnd Fragment', 'Fragment Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/fragsub/${userID}#Ralysnd Fragment', true)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <button onclick="openQR('https://${hostName}/fragsub/${userID}?app=hiddify#Ralysnd-Fragment', 'Fragment Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/fragsub/${userID}?app=hiddify#Ralysnd-Fragment', true)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
                    </tr>
			...
			<div class="table-container">
				<table id="normal-configs-table">
					...
					<tr>
						...
						<td>
                            <button onclick="openQR('https://${hostName}/warpsub/${userID}?app=xray#Ralysnd-Warp', 'Warp Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/warpsub/${userID}?app=xray#Ralysnd-Warp', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
					</tr>
					<tr>
						...
						<td>
                            <button onclick="openQR('sing-box://import-remote-profile?url=https://${hostName}/warpsub/${userID}?app=singbox#Ralysnd-Warp', 'Warp Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/warpsub/${userID}?app=singbox#Ralysnd-Warp', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
						</td>
					</tr>
                    <tr>
                        ...
                        <td>
                            <button onclick="openQR('https://${hostName}/warpsub/${userID}?app=clash#Ralysnd-WARP', 'Warp Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/warpsub/${userID}?app=clash#Ralysnd-WARP', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
                    </tr>
			...
			<div class="table-container">
				<table id="warp-pro-configs-table">
					...
					<tr>
						...
						<td>
                            <button onclick="openQR('https://${hostName}/warpsub/${userID}?app=nikang#Ralysnd-Warp-Pro', 'Warp Pro Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/warpsub/${userID}?app=nikang#Ralysnd-Warp-Pro', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
					</tr>
					<tr>
						...
						<td>
                            <button onclick="openQR('sing-box://import-remote-profile?url=https://${hostName}/warpsub/${userID}?app=hiddify#Ralysnd-Warp-Pro', 'Warp Pro Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/warpsub/${userID}?app=hiddify#Ralysnd-Warp-Pro', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
						</td>
					</tr>
            ...
            <div class="footer">
                ...
                <a class="link" href="https://github.com/bia-pain-bache/Ralysnd-Worker-Panel" target="_blank">Github</a>
    ...
    <body>
        <div id="error-container">
            ...
            <div id="error-message">
                <h2>${message} ${refer 
                    ? 'Please try again or refer to <a href="https://github.com/bia-pain-bache/Ralysnd-Worker-Panel/blob/main/README.md">documents</a>' 
                    : ''}
                </h2>
...
async function fetchWgConfig (env, warpKeys) {
    ...
    await env.Ralysnd.put('warpConfigs', JSON.stringify(warpConfigs));
}
...
async function buildWorkerLessConfig(remoteDNS, localDNS, lengthMin,  lengthMax,  intervalMin,  intervalMax, blockAds, bypassIran, blockPorn, bypassLAN, bypassChina, blockUDP443) {
    ...
    fragConfig.remarks  = ' Ralysnd F - WorkerLess ⭐'
...
async function getFragmentConfigs(env, hostName) {
    ...
    if (outProxy) {
        ...
        try {
            ...
            await env.Ralysnd.put("proxySettings", JSON.stringify({
                ...proxySettings, 
                outProxy: '',
                outProxyParams: ''
            }));
    ...
    bestPing.remarks = ' Ralysnd F - Best Ping 💥';
    ...
    bestFragment.remarks = ' Ralysnd F - Best Fragment 😎';
...
async function getXrayWarpConfigs (env, client) {
    ...
    xrayWarpBestPing.remarks = client === 'nikang' ? ' Ralysnd - Warp Pro Best Ping 🚀' : ' Ralysnd - Warp Best Ping 🚀';
    ...
    xrayWarpOutbounds.forEach((outbound, index) => {
        xrayWarpConfigs.push({
            ...xrayWarpConfig,
            remarks: client === 'nikang' ? ` Ralysnd - Warp Pro ${index + 1} 🇮🇷` : ` Ralysnd - Warp ${index + 1} 🇮🇷`,
    ...
    xrayWoWOutbounds.forEach((outbound, index) => {
        if (outbound.tag.includes('warp-out')) {
            ...
            xrayWoWConfig.remarks = client === 'nikang' ? ` Ralysnd - WoW Pro ${index/2 + 1} 🌍` : ` Ralysnd - WoW ${index/2 + 1} 🌍`;
    ...
    xrayWoWBestPing.remarks = client === 'nikang' ? ' Ralysnd - WoW Pro Best Ping 🚀' : ' Ralysnd - WoW Best Ping 🚀';
